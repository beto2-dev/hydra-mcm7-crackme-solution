...
static uint32_t finish_lane(const uint8_t *kt, uint32_t *r11_out, uint8_t *cb_out) {
    ...
    /* the verdict FNV runs over the full unzeroed buffer; the byte at
     * index 28 is zeroed by the real code only AFTER the comparison
     * (0x5ECA4), so it must not be touched here */
    *r11_out = r11;
